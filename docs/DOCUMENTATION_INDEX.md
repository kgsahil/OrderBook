# Documentation Index

Complete guide to all project documentation.

## Quick Start Guides

- **[QUICKSTART.md](../QUICKSTART.md)** - Get started in 5 minutes (Docker & Manual)
- **[DOCKER_QUICKSTART.md](DOCKER_QUICKSTART.md)** - Docker-specific quick start
- **[DOCKER_SEPARATE.md](DOCKER_SEPARATE.md)** - Running components separately

## Architecture & Design

- **[PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md)** - Complete project organization
- **[ARCHITECTURE.md](ARCHITECTURE.md)** - System architecture and design patterns
- **[ARCHITECTURE_FLOW.md](ARCHITECTURE_FLOW.md)** - Detailed service communication flows and event diagrams
- **[API_CONTRACT.md](API_CONTRACT.md)** - Inter-component communication protocols
- **[API_REFERENCE.md](API_REFERENCE.md)** - WebSocket and REST API reference

## Setup & Deployment

- **[DOCKER_SETUP.md](DOCKER_SETUP.md)** - Complete Docker deployment guide
- **[RUNNING_SERVICES.md](RUNNING_SERVICES.md)** - Manual service management
- **[TROUBLESHOOTING.md](TROUBLESHOOTING.md)** - Common issues and solutions (NEW - Comprehensive troubleshooting guide)

## Component Documentation

- **[orderbook/README.md](../orderbook/README.md)** - OrderBook component
- **[dashboard/README.md](../dashboard/README.md)** - Dashboard component
- **[agents/README.md](../agents/README.md)** - Agents component

## Development

- **[CONTRIBUTING.md](../CONTRIBUTING.md)** - Contribution guidelines
- **[INTERVIEW_GUIDE.md](INTERVIEW_GUIDE.md)** - Interview preparation guide

## Reference

- **[REFACTORING_SUMMARY.md](REFACTORING_SUMMARY.md)** - Component segregation details
- **[CLEANUP_SUMMARY.md](CLEANUP_SUMMARY.md)** - Cleanup and documentation updates

## Quick Navigation

| I want to... | Read... |
|--------------|---------|
| Get started quickly | [QUICKSTART.md](../QUICKSTART.md) |
| Understand the architecture | [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) |
| Deploy with Docker | [DOCKER_SETUP.md](DOCKER_SETUP.md) |
| Learn the API | [API_CONTRACT.md](API_CONTRACT.md) |
| Troubleshoot issues | [TROUBLESHOOTING.md](TROUBLESHOOTING.md) |
| Contribute code | [CONTRIBUTING.md](../CONTRIBUTING.md) |

